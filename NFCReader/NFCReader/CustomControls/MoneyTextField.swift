...
open class MoneyTextField: UIControl, UITextViewDelegate, InternalTextViewDelegate {
    ...
    open var amount: Double {
        get {
            let number = NSDecimalNumber(string: self.rawString, locale: self.locale)
            if number.stp_isNAN() {
                return 0
            } else {
        ...
        set(newAmount) {
            let number = NSDecimalNumber(value: newAmount)
    ...
    open var currency: String = "usd" {
        didSet(oldValue) {
            ...
            let number = NSDecimalNumber(string: self.rawString, locale: locale)
    ...
    open var largeFont: UIFont = UIFont.systemFont(ofSize: 100) {
        didSet { self.refreshAttributedString() }
    }
    open var smallFont: UIFont = UIFont.systemFont(ofSize: 50) {
        didSet { self.refreshAttributedString() }
    }
    @IBInspectable
    ...
    fileprivate var zero: String {
        ...
        return self.formatNumber(number, hideCentsIfZero: true)
    }
    ...
    open func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        ...
        if deleting {
            if newRawString.count > 0 {
                ...
                newRawString = String(newRawString[..<lastIndex]).trimmingCharacters(in: .whitespaces)
            ...
            if text == self.decimalSeparator && !self.hasDecimalSeparator {
